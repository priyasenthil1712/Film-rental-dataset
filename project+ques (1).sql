...
select * from film;
select title,char_length(title) as char_len, rental_rate from film order by char_len desc limit 1;
...
use film_rental;
select * from film;
select title,length,rental_date
...
from actor join film_actor using(actor_id) join film using(film_id) where description like '%wrestler%';
...
with cte1 as (select distinct c.customer_id,c.first_name,c.last_name,sum(amount) 
over(partition by customer_id) as rental_payment 
...
group by c.customer_id, rental_date, first_name, title, rental_duration, amount;

-- 17. Display the customers who paid 50% of their total rental costs within one day

use film_rental;
select c.customer_id,f.film_id,(f.rental_rate*f.rental_duration) as rental_cost,p.amount,
(p.amount/(f.rental_rate * f.rental_duration))*100 as pct_paid  from film f 
join inventory i using (film_id) join rental r using (inventory_id) 
join customer c using (customer_id)  join payment p using (rental_id) 
